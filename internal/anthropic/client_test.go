...
func TestCreateMessageStreamReportsToolArgumentActivity(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte(strings.Join([]string{
			`event: message_start`,
			...
			``,
			`event: content_block_delta`,
			`data: {"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"q\":\"sdk\"}"}}`,
			``,
			`event: content_block_stop`,
			`data: {"type":"content_block_stop","index":0}`,
			``,
	...
	ctx := modelactivity.WithSink(context.Background(), func() { activityCount.Add(1) })
	client := NewClient("test-key", WithBaseURL(srv.URL))
	stream, err := client.CreateMessageStream(ctx, CreateMessageRequest{
		Model:     "claude-sonnet-4-5",
		MaxTokens: 8,
		Messages: []Message{{
			Role:    RoleUser,
	...
	if visibleEvents == 0 {
		t.Fatal("stream produced no visible events")
	}
	if got := activityCount.Load(); got == 0 {
		t.Fatal("provider response bytes, including tool arguments, reported no activity")
	}
}
