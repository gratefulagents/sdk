...
- `ApplyPatch` accepts `patch` as either a bounded unified Git-style diff or an OpenAI patch envelope, plus optional `dry_run`. OpenAI envelopes use `*** Begin Patch`/`*** End Patch`, `*** Update/Add/Delete File: path`, direct change chunks or range-less `@@`/`@@ locator` hunks with context, `+`, and `-` lines, optional `*** Move to: path`, and `*** End of File`. Unified diffs support file creation, modification, deletion, rename metadata, and executable-bit changes; OpenAI envelopes support creation, modification, deletion, and moves. The complete patch, all paths, source contents, modes, and hunks are validated before mutation. Unsupported directives (including copy and binary patches) fail closed. Results are JSON with `dry_run`, per-file `operations`, and a bounded human-readable `diff`.
